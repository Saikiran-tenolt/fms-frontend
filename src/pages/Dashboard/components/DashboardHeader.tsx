...
export const DashboardHeader: React.FC<DashboardHeaderProps> = React.memo(({
  ...
  // FIX: Stable close handler — avoids recreating the fn on every render.
  // Dropdown's useEffect([open, onClose]) re-attaches a document mousedown
  // listener whenever onClose changes identity. An inline arrow is a new
  // reference every render, so the listener would be torn down and re-added
  // on every parent update. useCallback keeps it stable.
  const closePlot = useCallback(() => setPlotOpen(false), []);
