...
type Stage = 'PHONE' | 'OTP' | 'ADMIN_CORE';
...
export const LoginPage: React.FC = () => {
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.includes('E11000')) {
        if (msg.includes('email')) msg = 'Email already taken';
    ...
    try {
      ...
      if (res.success) {
        if (res.data.type === 'SIGNUP_REQUIRED') {
          ...
          if (resendRes.otp) console.warn('DEV — Fresh signup OTP:', resendRes.otp);
          setResendTimer(60);
          setOtp(Array(6).fill(''));
        } else {
          dispatch(login({
  ...
  // ── Admin login — calls POST /api/v1/admin/login ────────────────────────────
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store admin token — backend uses a single `token` field (no refresh token)
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="relative overflow-hidden rounded-3xl border p-10 shadow-[0_4px_24px_rgba(0,0,0,0.02)]"
              ...
              <motion.div key="phone" initial={{ x: -12, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -12, opacity: 0 }}>
                ...
                <form onSubmit={handleSendOtp} className="space-y-5">
                  <div>
                    ...
                    {fieldErrors.phone && (
                      <p className="mt-1.5 ml-1 text-[11px] font-semibold text-red-500 flex items-center gap-1.5">
              ...
              <motion.div key="otp" initial={{ x: 12, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 12, opacity: 0 }}>
                      ...
                      <div className="space-y-3">
                        <div className="relative">
                          ...
                          <input type="text" value={name} onChange={e => { setName(e.target.value); setFieldErrors(p => ({ ...p, name: '' })); }} placeholder="Full name"
                            className={`w-full rounded-[9px] border py-3 pl-10 pr-4 text-sm outline-none transition-all ${fieldErrors.name ? 'border-red-400 bg-red-50 text-red-900' : ''}`}
                        ...
                        <div className="relative">
                          ...
                          <input type="email" value={email} onChange={e => { setEmail(e.target.value); setFieldErrors(p => ({ ...p, email: '' })); }} placeholder="Email address"
                            className={`w-full rounded-[9px] border py-3 pl-10 pr-4 text-sm outline-none transition-all ${fieldErrors.email ? 'border-red-400 bg-red-50 text-red-900' : ''}`}
                        ...
                        {isPinVerified && (
                          <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 gap-2">
                            ...
                            <input value={location.village} onChange={e => setLocation({ ...location, village: e.target.value })}
                              ...
                              className="rounded-[6px] border px-3 py-2.5 text-xs outline-none"
                ...
                <div className="mt-5 flex items-center justify-between border-t pt-4" style={{ borderColor: '#e2e8f0' }}>
                  ...
                  <Button
                    ...
                    style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: '#ffffff', border: 'none', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.2)' }}
                  >
                    {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : isSignup ? 'Create account' : 'Verify & enter'}
                  </Button>
                </div>
              </motion.div>
            )}

            {/* ── ADMIN CORE STAGE — email + password → real API ── */}
            {stage === 'ADMIN_CORE' && (
              <motion.div key="admin" initial={{ x: 12, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 12, opacity: 0 }}>
                <h1 className="mb-1.5 text-2xl font-semibold leading-tight" style={{ color: '#0f172a' }}>
                  Administrative<br />access
                </h1>
                <p className="mb-4 text-[13px] leading-relaxed" style={{ color: '#64748b' }}>
                  Government credentials required
                </p>

                {/* Demo Credentials Box */}
                <div className="mb-6 rounded-xl border p-3 text-xs" style={{ background: '#f8fafc', borderColor: '#e2e8f0' }}>
                  <p className="mb-1.5 font-semibold flex items-center gap-1.5" style={{ color: '#0f172a' }}>
                    <Info className="h-3.5 w-3.5" style={{ color: '#10b981' }} /> Demo Access
                  </p>
                  <p className="font-mono text-[11px] mb-1" style={{ color: '#64748b' }}>
                    Email: <span style={{ color: '#0f172a' }}>admin@example.com</span>
                  </p>
                  <p className="font-mono text-[11px]" style={{ color: '#64748b' }}>
                    Password: <span style={{ color: '#0f172a' }}>admin@123</span>
                  </p>
                </div>
                ...
                <form onSubmit={handleAdminLogin} className="space-y-3">
                  ...
                  >
                    {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Authorize account →'}
                  </Button>
                </form>
