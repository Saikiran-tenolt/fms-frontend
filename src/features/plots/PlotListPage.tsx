...
// ── WHY callbacks are inside PlotCard (not passed as props from parent) ───────
// Passing onClick/onEdit/onDelete as arrow-function props from PlotListPage
// created new function references on every parent render. That defeated
// React.memo on PlotCard because memo does a shallow-equality check on props.
// By moving navigate() and dispatch() calls inside the memoized PlotCard,
// all callbacks are created once per card mount and never change — React.memo
// can now do its job: cards only re-render when plot data or sensorData changes.
...
const PlotCard = React.memo(function PlotCard({ plot, sensorData }: PlotCardProps) {
  ...
  // Stable callbacks — these are created once per card mount, never change
...
export const PlotListPage: React.FC = () => {
  console.count('PlotListPage rendered');

  const navigate = useNavigate();
  const dispatch = useAppDispatch();

  // ── FIX: Granular selectors — same pattern as DashboardPage ──────────────
  ...
  const sensorData = useAppSelector((state) => state.sensors.sensorData);
  ...
  // Seed sensor mock data for any plots that don't have it yet
  useEffect(() => {
  ...
  }, [plots, dispatch]); // sensorData intentionally omitted
